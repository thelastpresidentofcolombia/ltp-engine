/**
 * Engine Icon Registry
 * Lightweight SVG strings to replace emojis.
 * Matches Lucide style (24x24, stroke-width 2).
 */
...
export const EngineIcons = {
  ...
  'wave': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 11.5V9a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v1.4"/><path d="M14 10V8a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v2"/><path d="M10 10.5V6a2 2 0 0 0-2-2v0a2 2 0 0 0-2 2v8"/><path d="M18 8a2 2 0 1 1 4 0v6a8 8 0 0 1-8 8h-2c-2.8 0-4.5-.86-5.99-2.34l-3.6-3.6a2 2 0 0 1 2.83-2.82L7 15"/></svg>`,
  ...
  'alert': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`,
  ...
  'video': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m16 13 5.223 3.482a.5.5 0 0 0 .777-.416V7.87a.5.5 0 0 0-.777-.416L16 10"/><rect x="2" y="6" width="14" height="12" rx="2"/></svg>`,
  ...
  // Profile / Vertical
  'pencil': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>`,
  ...
  'log-out': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>`,
  ...
  'trending-down': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"/><polyline points="16 17 22 17 22 11"/></svg>`,
  ...
  'check-circle': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>`,
  'x-circle': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>`,
  'arrow-left': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>`,
  'external-link': `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/></svg>`,
...
/** Return an SVG string with optional extra classes */
export function getIcon(name: keyof typeof EngineIcons, className = 'w-icon-svg'): string {
  const svg = EngineIcons[name] || EngineIcons['plus'];
  // Inject class into the svg tag
  return svg.replace('<svg', `<svg class="${className}"`);
}

/** Standard empty-state HTML block (A-grade: icon + title + desc + optional CTA) */
export function emptyStateHtml(opts: {
  icon: keyof typeof EngineIcons;
  title: string;
  desc?: string;
  action?: { label: string; href?: string; id?: string };
}): string {
  const iconHtml = getIcon(opts.icon, 'empty-state-svg');
  const descHtml = opts.desc ? `<div class="empty-state-desc">${opts.desc}</div>` : '';
  let actionHtml = '';
  if (opts.action) {
    const tag = opts.action.href ? 'a' : 'button';
    const href = opts.action.href ? ` href="${opts.action.href}"` : '';
    const id = opts.action.id ? ` id="${opts.action.id}"` : '';
    actionHtml = `<div class="empty-state-action"><${tag} class="btn-primary btn-sm"${href}${id}>${opts.action.label}</${tag}></div>`;
  }
  return `<div class="empty-state"><div class="empty-state-icon">${iconHtml}</div><div class="empty-state-title">${opts.title}</div>${descHtml}${actionHtml}</div>`;
}

/** Standard error-state HTML block */
export function errorStateHtml(opts: {
  title?: string;
  desc?: string;
  retry?: { label?: string; id?: string };
}): string {
  const iconHtml = getIcon('alert', 'empty-state-svg');
  const title = opts.title || 'Something went wrong';
  const desc = opts.desc || 'Please try again or refresh the page.';
  let retryHtml = '';
  if (opts.retry) {
    const id = opts.retry.id ? ` id="${opts.retry.id}"` : '';
    retryHtml = `<div class="empty-state-action"><button class="btn-ghost btn-sm"${id}>${opts.retry.label || 'Try again'}</button></div>`;
  }
  return `<div class="empty-state empty-state-error"><div class="empty-state-icon">${iconHtml}</div><div class="empty-state-title">${title}</div><div class="empty-state-desc">${desc}</div>${retryHtml}</div>`;
}
